...
class Position(Base):
    ...
    stop_order_id = Column(String, nullable=True)  # Track stop order
    stop_price = Column(Float, nullable=True)     # Track stop price
...
def calculate_technical_indicators(symbols, lookback_days=200):
    ...
    if historical_data.empty or len(historical_data) < 35:
...
@sleep_and_retry
@limits(calls=CALLS, period=PERIOD)
def calculate_rsi(symbols, period=14, interval='1d'):
    print(f"Calculating RSI for {symbols} (period={period}, interval={interval}) using yfinance...")
    yf_symbol = symbols.replace('.', '-')
    stock_data = yf.Ticker(yf_symbol)
    historical_data = stock_data.history(period='20d', interval=interval)
...
def sell_stocks(symbols_to_sell_dict, buy_sell_lock):
    ...
    for pos in positions:
        ...
        macd_color = GREEN if latest_macd and latest_macd_signal and latest_macd >= latest_macd_signal else RED
        signal_color = GREEN if latest_macd_signal and latest_macd_signal >= 0 else RED
        print(f"MACD: {macd_color}{latest_macd:.4f if latest_macd else 'N/A'}{RESET}, Signal: {signal_color}{latest_macd_signal:.4f if latest_macd_signal else 'N/A'}{RESET}, MACD below signal: {macd_below_signal}")
        rsi = talib.RSI(close, timeperiod=14)
        latest_rsi = rsi[-1] if len(rsi) > 0 and not np.isnan(rsi[-1]) else None
        rsi_color = GREEN if latest_rsi and latest_rsi >= 50 else RED
        print(f"RSI: {rsi_color}{latest_rsi:.2f if latest_rsi else 'N/A'}{RESET}")
        atr = get_average_true_range(symbol)
        stop_loss_price = avg_entry_price * (1 - 2 * atr / avg_entry_price) if atr else None
        stop_loss_triggered = current_price <= stop_loss_price if stop_loss_price else False
        stop_color = GREEN if stop_loss_price and stop_loss_price >= 0 else RED
        print(f"Stop-loss price: {stop_color}${stop_loss_price:.2f if stop_loss_price else 'N/A'}{RESET}, Triggered: {stop_loss_triggered}")
        if macd_below_signal or (latest_rsi and latest_rsi > 70) or stop_loss_triggered:
            reason = []
            if macd_below_signal:
                reason.append("MACD below signal")
            if latest_rsi and latest_rsi > 70:
                reason.append("RSI > 70")
            if stop_loss_triggered:
                reason.append("Stop-loss triggered")
            reason_str = ", ".join(reason)
            print(f"{RED}SELL SIGNAL for {symbol}: {reason_str}{RESET}")
            with buy_sell_lock:
                if ensure_no_open_orders(symbol):
                    ...
                    if order_id:
                        ...
                        if status_info and status_info["status"] == "FILLED":
                            ...
                            filled_price = status_info["avg_price"] or current_price
                            filled_color = GREEN if filled_price >= 0 else RED
                            print(f"SELL ORDER FILLED for {filled_qty:.4f} shares of {symbol} at {filled_color}${filled_price:.2f}{RESET}")
                            ...
                            try:
                                ...
                                symbols_to_sell_dict[symbol] = filled_qty
                                send_alert(
                                    f"Sold {filled_qty:.4f} shares of {symbol} at ${filled_price:.2f} ({reason_str})",
                                    subject=f"Sell Executed: {symbol}",
                                    use_sms=True
                                )
                            except Exception as e:
                                logging.error(f"Error updating database/CSV for sell {symbol}: {e}")
                            finally:
                                session.close()
                    else:
                    ...
                    logging.warning(f"Open orders exist for {symbol}.")
        else:
            print(f"No sell signal for {symbol}.")
    print("Completed sell_stocks function.")
...
def main():
    ...
    if not symbols_to_buy_list:
        ...
        return
    schedule.every(5).minutes.do(lambda: stop_if_stock_market_is_closed())
    schedule.every(2).minutes.do(buy_stocks, symbols_to_sell_dict, symbols_to_buy_list, buy_sell_lock)
    schedule.every(3).minutes.do(sell_stocks, symbols_to_sell_dict, buy_sell_lock)
